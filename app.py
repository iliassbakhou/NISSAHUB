...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'user_id' not in session: return redirect(url_for('login_page', next=request.url))
...
def manage_courses_page():
    try:
        # === THE FIX: Changed collection_group('skills') to collection('skills') ===
        ...
        for course_doc in courses_query:
            ...
            # Fetch author name from cache or Firestore
            ...
            course_data['author_name'] = users_cache.get(author_id, {}).get('displayName', 'Unknown')
        ...
        traceback.print_exc()
        return render_template('admin/manage_courses.html', page_title="Manage Courses", courses=[])
...
def marketplace_page():
    try:
        ...
        for doc in products_query:
            ...
            product_data['author'] = users_cache.get(author_id)
            products_list.append(product_data)
            
        return render_template('products/marketplace.html', products=products_list, page_title="Marketplace")
    except Exception:
        traceback.print_exc()
        flash("Could not load the marketplace. An admin may need to configure database indexes.", "error")
        return render_template('products/marketplace.html', products=[], page_title="Marketplace")
...
def product_detail_page(product_id):
    try:
        ...
        if not product_doc.exists:
            flash("Sorry, this product could not be found.", "error")
            return redirect(url_for('marketplace_page'))

        product_data = {'id': product_doc.id, **product_doc.to_dict()}
        
        current_user = inject_user_data().get('current_user', {})
        is_admin = current_user.get('isAdmin', False)
        is_author = product_data.get('author_id') == session.get('user_id')

        if not product_data.get('isPublished', False) and not (is_admin or is_author):
            flash("Sorry, this product is not currently available.", "error")
            return redirect(url_for('marketplace_page'))
        ...
        traceback.print_exc()
        flash("An error occurred while trying to load the product page.", "error")
        return redirect(url_for('marketplace_page'))
...
@app.route('/skill/<string:skill_id>', methods=['GET', 'POST'])
@login_required
def skill_detail_page(skill_id):
    try:
        ...
        current_user = inject_user_data().get('current_user') or {}; is_admin, is_author = current_user.get('isAdmin', False), skill_data.get('author_id') == session.get('user_id')
        ...
        if request.method == 'POST':
            rating, review_text = request.form.get('rating'), request.form.get('review_text', '').strip()
            if not rating or not review_text: flash("Rating and review text are required.", "error"); return redirect(url_for('skill_detail_page', skill_id=skill_id))
            skill_ref.collection('reviews').add({'user_id': session['user_id'], 'rating': int(rating), 'text': review_text, 'created_at': datetime.datetime.now(tz=datetime.timezone.utc), 'skill_id': skill_id})
            flash("Review submitted. Thank you!", "success"); return redirect(url_for('skill_detail_page', skill_id=skill_id))
        ...
        reviews_list, total_rating = [], 0
        for doc in skill_ref.collection('reviews').order_by('created_at', direction=firestore.Query.DESCENDING).stream():
            review_data = doc.to_dict(); total_rating += review_data.get('rating', 0)
            if user_id := review_data.get('user_id'): review_data['user_profile'] = db.collection('users').document(user_id).get().to_dict() or {}
            reviews_list.append(review_data)
        review_summary = {"count": len(reviews_list), "average": round(total_rating / len(reviews_list), 1) if reviews_list else 0}
        
        discussions_list = []
        for post_doc in skill_ref.collection('discussions').order_by('created_at', direction=firestore.Query.ASCENDING).stream():
            post_data = {'id': post_doc.id, **post_doc.to_dict()}
            if user_id := post_data.get('user_id'): post_data['user_profile'] = db.collection('users').document(user_id).get().to_dict() or {}
            post_data['replies'] = []
            for reply_doc in post_doc.reference.collection('replies').order_by('created_at', direction=firestore.Query.ASCENDING).stream():
                reply_data = {'id': reply_doc.id, **reply_doc.to_dict()}
                if reply_user_id := reply_data.get('user_id'): reply_data['user_profile'] = db.collection('users').document(reply_user_id).get().to_dict() or {}
                post_data['replies'].append(reply_data)
            discussions_list.append(post_data)
    ...
    except Exception: traceback.print_exc(); flash("An error processing your request.", "error"); return redirect(url_for('skills_page'))
...
def creator_profile_page(creator_id):
    ...
    except Exception: flash("An error occurred loading the creator's profile. An admin may need to create a database index.", "error"); traceback.print_exc(); return redirect(url_for('skills_page'))

# ===== NEW CUSTOMER PROFILE ROUTE =====
...
def customer_profile_page(user_id):
    try:
        ...
        # Security Check: Ensure the user exists and is a customer
        if not user_doc.exists or user_doc.to_dict().get('role') != 'customer':
            flash("User profile not found or this user is not a customer.", "error")
            return redirect(url_for('home'))

        profile_user = user_doc.to_dict()
        
        # Fetch this user's public activity, e.g., their reviews.
        # This uses a collection group query to find reviews across all skills.
        ...
        activity_list = []
        skill_cache = {} # Cache to avoid fetching the same skill multiple times
        for review_doc in reviews_query:
            review_data = review_doc.to_dict()
            skill_id = review_data.get('skill_id')
            
            # Fetch the skill data associated with the review
            if skill_id:
                if skill_id not in skill_cache:
                    skill_doc = db.collection('skills').document(skill_id).get()
                    skill_cache[skill_id] = skill_doc.to_dict() if skill_doc.exists else None
        ...
        return render_template('users/profile_page.html', 
                               ...
                               page_title=f"Profile for {profile_user.get('displayName')}")

    except Exception as e:
        flash("An error occurred trying to load the profile.", "error")
        traceback.print_exc()
        return redirect(url_for('home'))
...
def course_player_page(skill_id, lesson_id):
    try:
        ...
        previous_lesson = all_lessons_list[active_lesson_index - 1] if active_lesson_index > 0 else None
        next_lesson = all_lessons_list[active_lesson_index + 1] if active_lesson_index < len(all_lessons_list) - 1 else None
    ...
    except Exception: traceback.print_exc(); flash("There was an error loading the course.", "error"); return redirect(url_for('skills_page'))
...
# ===== SHARED OWNERSHIP CHECK FUNCTIONS =====
def check_skill_ownership(skill_id, user_id):
    skill_ref, skill_doc = db.collection('skills').document(skill_id), db.collection('skills').document(skill_id).get()
    if not skill_doc.exists:
        flash("Course not found.", "error"); return None, None, redirect(url_for('my_skills_page'))
    skill_data = skill_doc.to_dict()
    if skill_data.get('author_id') != user_id:
        flash("You can only manage your own courses.", "error"); return None, None, redirect(url_for('my_skills_page'))
...
def check_product_ownership(product_id, user_id):
    ...
    if not product_doc.exists:
        flash("Product not found.", "error"); return None, None, redirect(url_for('my_products_page'))
    product_data = product_doc.to_dict()
    if product_data.get('author_id') != user_id:
        flash("You can only manage your own products.", "error"); return None, None, redirect(url_for('my_products_page'))
    product_data['id'] = product_id 
    return product_ref, product_data, None

# ===== CREATOR-SIDE SKILL (COURSE) MANAGEMENT =====
@app.route('/my-skills')
@login_required
def my_skills_page():
...
def create_skill_page():
    ...
    if request.method == 'POST':
        ...
        name, desc, cat = request.form.get('skill_name'), request.form.get('skill_description'), request.form.get('skill_category')
        ...
        if 'skill_image' in request.files and request.files['skill_image'].filename != '':
            ...
            try: 
                res = cloudinary.uploader.upload(image_file, folder="nissahub_skills", transformation=[{'width': 1000, 'height': 750, 'crop': 'limit'}]); image_url = res.get('secure_url')
        ...
        try:
            skill_data = { 'name': name, 'description': desc, 'category': cat, 'author_id': session['user_id'], 'author_email': session.get('email'), 'created_at': datetime.datetime.now(tz=datetime.timezone.utc), 'image_url': image_url, 'search_tokens': generate_search_tokens(f"{name} {desc}"), 'isPublished': is_published, 'isFeatured': False }
...
def edit_skill_page(skill_id):
    ...
    if request.method == 'POST':
        updated_data = { 'name': request.form.get('skill_name'),'description': request.form.get('skill_description'), 'category': request.form.get('skill_category'), 'updated_at': datetime.datetime.now(tz=datetime.timezone.utc), 'search_tokens': generate_search_tokens(f"{request.form.get('skill_name')} {request.form.get('skill_description')}"), 'isPublished': request.form.get('is_published') == 'true' }
        if 'skill_image' in request.files and request.files['skill_image'].filename != '':
            image_file = request.files.get('skill_image')
            try:
...
def delete_skill(skill_id):
    ...
    skill_ref.delete()
    flash(f"Skill '{skill_data.get('name')}' deleted.", 'success'); return redirect(url_for('my_skills_page'))

# --- CREATOR-SIDE PRODUCT (MARKETPLACE) MANAGEMENT ---
...
def create_product_page():
    ...
    if request.method == 'POST':
        ...
        try:
            new_product_data = {'name': form_data['name'], 'description': form_data['description'], 'price': float(form_data['price']), 'category': form_data['category'], 'isPublished': form_data['isPublished'], 'image_url': image_url, 'author_id': session['user_id'], 'author_email': session.get('email'), 'created_at': datetime.datetime.now(tz=datetime.timezone.utc), 'isFeatured': False }
...
def edit_product_page(product_id):
    ...
    if request.method == 'POST':
        updated_data = {
            'name': request.form.get('product_name'), 'description': request.form.get('product_description'), 'price': float(request.form.get('product_price')),
            'category': request.form.get('product_category'), 'isPublished': request.form.get('is_published') == 'true', 'updated_at': datetime.datetime.now(tz=datetime.timezone.utc)
        }
        if 'product_image' in request.files and request.files['product_image'].filename != '':
            image_file = request.files['product_image']
            try:
        ...
        flash(f'Product "{updated_data["name"]}" updated successfully!', 'success')
...
def delete_product(product_id):
    ...
    try:
        if 'cloudinary' in (img_url := product_data.get('image_url', '')):
            if public_id := get_public_id_from_url(img_url): cloudinary.uploader.destroy(public_id)
        product_ref.delete()
        flash(f"Product '{product_data.get('name')}' has been deleted successfully.", 'success')
    except Exception as e:
        traceback.print_exc()
        flash("An error occurred while trying to delete the product.", 'error')
    return redirect(url_for('my_products_page'))
...
def manage_lessons_page(skill_id):
    ...
    if request.method == 'POST':
        ...
        lessons_ref.add({'title': title, 'lesson_type': l_type, 'content': content, 'created_at': datetime.datetime.now(tz=datetime.timezone.utc), 'order': next_order})
...
def edit_lesson_page(skill_id, lesson_id):
    ...
    if request.method == 'POST':
        title, l_type = request.form.get('lesson_title'), request.form.get('lesson_type')
        content = request.form.get('content_text', '') if l_type == "Text" else request.form.get('content_video', '')
        lesson_ref.update({'title': title, 'lesson_type': l_type, 'content': content, 'updated_at': datetime.datetime.now(tz=datetime.timezone.utc)})
...
def select_role_page():
    if request.method == 'POST':
        ...
        try:
            ...
            user_data = {'uid': user_id, 'email': email, 'role': role, 'createdAt': datetime.datetime.now(tz=datetime.timezone.utc), 'displayName': f"user_{user_id[:6]}"}
